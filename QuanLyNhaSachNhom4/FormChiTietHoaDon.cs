...
namespace QuanLyCuaHangBanSach
{
    public partial class FormChiTietHoaDon : Form
    {
        ...
        private void loadDgHoaDon()
        {
            ...
            query.Append(",tbl_chi_tiet_hoa_don.gia_ban as [Giá Bán]");
            query.Append(",tbl_chi_tiet_hoa_don.so_luong * tbl_chi_tiet_hoa_don.gia_ban as [Thành Tiền]");
        ...
        private void loadTongTien()
        {
            ...
            {
                StringBuilder query = new StringBuilder("SELECT SUM(tbl_chi_tiet_hoa_don.so_luong * tbl_chi_tiet_hoa_don.gia_ban)");
                query.Append(" FROM tbl_chi_tiet_hoa_don");
                query.Append(" WHERE tbl_chi_tiet_hoa_don.ma_hoa_don = " + maHoaDon);
        ...
        private void btnThem_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (dem == 0)
                {
                    float giaBan = float.Parse(dataProvider.execScaler("SELECT gia_ban FROM tbl_sach WHERE ma_sach = " + maSach).ToString());

                    StringBuilder query = new StringBuilder("EXEC proc_them_chi_tiet_hoa_don");
                    query.Append(" @maHoaDon = " + maHoaDon);
                    query.Append(",@maSach = " + maSach);
                    query.Append(",@soLuong = " + numSoLuongSach.Value);
                    query.Append(",@giaBan = " + giaBan);
